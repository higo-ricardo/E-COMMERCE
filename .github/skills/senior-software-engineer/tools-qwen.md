# Ferramentas Disponíveis — Qwen Code

> Este arquivo mapeia todas as ferramentas internas e externas que o **Qwen Code** pode utilizar ao executar a skill `senior-software-engineer`.
...
| **Skill** | `skill` | Invocar skills especializados | `skill("code-reviewer")` |
...
## Mapeamento: Etapa da Skill → Ferramentas Qwen
...
8. **`skill` para invocar skills especializadas** — ex.: `skill("review")` para revisão de código
9. **`save_memory` para preferências do usuário** — ex.: idioma de output, convenções do projeto
